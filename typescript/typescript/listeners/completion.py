...
class CompletionEventListener:
    ...
    def on_query_completions(self, view, prefix, locations):
        ...
        """
        info = get_info(view)
        print('query completion:', info)
        if info:
            info.completion_prefix_sel = decrease_locs_to_regions(
                locations, len(prefix))
            if not IS_ST2:
                view.add_regions("apresComp", decrease_locs_to_regions(
                    locations, 0), flags=sublime.HIDDEN)
            ...
            if (not self.completions_ready) or IS_ST2:
                ...
                if IS_ST2:
                    ...
                    if locations[0] > 0:
                        prev_char = view.substr(sublime.Region(
                            locations[0] - 1, locations[0] - 1))
                        self.if_completion_request_member = (prev_char == ".")
                    else:
                        self.if_completion_request_member = False
                    print('ts_prefix==> ', prefix)
                    cli.service.async_completions(
                        view.file_name(), location, prefix, self.handle_completion_info)
    ...
    def handle_completion_info(self, completions_resp):
        ...
        print('ts_response==>', completions_resp)
        self.pending_completions = []
        if not IS_ST2:
            view = active_view()
            loc = view.sel()[0].begin()
            prefix_length = len(self.completion_request_prefix)
            # Get the current content from the starting location to the cursor
            # location
            cur_str = view.substr(sublime.Region(
                self.completion_request_loc - prefix_length, loc))
        ...
        if completions_resp["success"] and (completions_resp["request_seq"] == self.completion_request_seq or IS_ST2):
            ...
            if raw_completions:
                for raw_completion in raw_completions:
                    ...
                    completion = (
                        name + "\t" + raw_completion["kind"], name.replace("$", "\\$"))
...
print('setting completion listener')
...
EventHub.subscribe("on_text_command_with_info",
                   listener.on_text_command_with_info)
EventHub.subscribe("on_modified_with_info", listener.on_modified_with_info)
EventHub.subscribe("on_selection_modified_with_info",
                   listener.on_selection_modified_with_info)
EventHub.subscribe("on_post_text_command_with_info",
                   listener.on_post_text_command_with_info)
