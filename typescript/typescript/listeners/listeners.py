...
class TypeScriptEventListener(sublime_plugin.EventListener):
    ...
    def on_modified_with_info(self, view, info):
        ...
        # A series state-updating for the info object to sync the file content
        # on the server
        info.modified = True
        ...
        if info.pre_change_sent:
            ...
            if last_command == "insert":
                if (
                    ...
                    # the last selection is not a highlighted selection
                    and info.prev_sel[0].empty()
                    ...
                    send_replace_changes_for_regions(
                        view, static_regions_to_regions(info.prev_sel), key)
                    # mark change as handled so that on_post_text_command
            ...
            # Issue:
            # https://github.com/Microsoft/TypeScript-Sublime-Plugin/issues/277
    ...
    def on_selection_modified(self, view):
        ...
        # Todo: why do we only check this here? anyway to globally disable the
        # listener for non-ts files
        if not is_apex(view):
            return
    ...
    def on_text_command_with_info(self, view, command_name, args, info):
        ...
        if command_name == "left_delete":
            # backspace
            send_replace_changes_for_regions(
                view, left_expand_empty_region(view.sel()), "")
        elif command_name == "right_delete":
            # delete
            send_replace_changes_for_regions(
                view, right_expand_empty_region(view.sel()), "")
        else:
        ...
        EventHub.run_listeners("on_text_command_with_info",
                               view, command_name, args, info)
    ...
    def on_post_text_command(self, view, command_name, args):
        ...
        if info:
            if not info.change_sent and info.modified:
                self.on_post_text_command_with_info(
                    view, command_name, args, info)
    ...
    def on_post_text_command_with_info(self, view, command_name, args, info):
        ...
        EventHub.run_listeners(
            "on_post_text_command_with_info", view, command_name, args, info)
